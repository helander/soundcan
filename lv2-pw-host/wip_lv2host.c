...
static struct context *context_new()
{
	struct context *c;
...
static struct context *context_ref()
{
	if (_context == NULL) {
...
static void *lv2_instantiate(const struct wip_descriptor *desc,
...
static void lv2_free(const struct wip_descriptor *desc)
{
	struct descriptor *d = (struct descriptor*)desc;
...
static const struct wip_descriptor *lv2_make_desc(struct wip_plugin *plugin, const char *name)
{
	...
	if (name)
		...
		desc->desc.name = strdup(lilv_node_as_string(lilv_plugin_get_name(p->p)));
	
	desc->desc.flags = 0;

	desc->desc.n_ports = lilv_plugin_get_num_ports(p->p);
	desc->desc.ports = calloc(desc->desc.n_ports, sizeof(struct wip_port));
	...
	for (i = 0; i < desc->desc.n_ports; i++) {
		...
		struct wip_port *fp = &desc->desc.ports[i];
...
static void lv2_unload(struct wip_plugin *plugin)
{
	struct plugin *p = (struct plugin *)plugin;
	context_unref(p->c);
	free(p);
...
SPA_EXPORT
struct wip_plugin *wip_plugin_load(const char *plugin_uri)
{
	struct context *c;
...
{
	lv2_atom_sequence_clear((LV2_Atom_Sequence *)port->atomBuffer);
}

void midiport_write(struct wip_port *port, uint8_t *mididata, int midibytes, long evtime, int *offset)
{
...
{
  assert(evbuf->buf.atom.type != evbuf->atom_Sequence || evbuf->buf.atom.size >= sizeof(LV2_Atom_Sequence_Body));
  return evbuf->buf.atom.type == evbuf->atom_Sequence  ? evbuf->buf.atom.size -  sizeof(LV2_Atom_Sequence_Body) : 0;
}
...
/*
{
  ...
  *data                                = NULL;
  ...
  LV2_Atom_Sequence* aseq = port->atomBuffer;
  LV2_Atom_Event*    aev = (LV2_Atom_Event*)((char*)LV2_ATOM_CONTENTS(LV2_Atom_Sequence, aseq) + *offset);

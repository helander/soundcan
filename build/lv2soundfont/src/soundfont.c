...
LV2_Handle lv2_instantiate(const struct LV2_Descriptor* descriptor, double sampleRate, const char* bundlePath, const LV2_Feature* const* features)
{
    ...
    char* filename = malloc(strlen(bundlePath) + 14 /* strlen("/soundfont.sf2") */ + 1);

    if (filename == NULL)
        goto cleanup_synth;

    strcpy(filename, bundlePath);
    strcat(filename, "/soundfont.sf2");
    ...
    for (count = 0; (preset = fluid_sfont_iteration_next(sfont)) != NULL;)
    {
        ...
        printf("\ncount %d  bank %d   prog %d  name [%s]",count,bp.bank,bp.prog,fluid_preset_get_name(preset));
        programs[count++] = bp;
    }
#endif
